 ...
 */
case class agu_compute(cfg: TTConfig, aw: Int, lw: Int, pw: Int, lane_num: Int) extends Component {
  val io = new Bundle {
  ...
  when(io.loop_ctrl.fire) {
    ...
    tok_oh_buf(w) := io.loop_ctrl.payload.oh(0).asUInt
    tok_ow_buf(w) := io.loop_ctrl.payload.ow(0).asUInt

    // take lane0 token_id as "packet id" for the whole batch
    when(tokCount === 0) {
      tok_id0 := io.loop_ctrl.payload.token_id(0).asUInt
    } otherwise {
  ...
  val startBatch = (!agu_busy) && (tokCount === U(lane_num)) && cfg_vld_reg
  ...
  when(agu_busy) {
    for (l <- 0 until lane_num) {
      ...
      io.load_reqs(l).payload.req_laneid   := U(l, lw bits)
      io.load_reqs(l).payload.req_packetid := tok_id0.resize(pw)
